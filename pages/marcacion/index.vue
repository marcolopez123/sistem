    ...
    <AdminTemplate :page="page" :modulo="modulo">
      <div slot="body">
        <div class="row justify-content-end">
          ...
          <!-- Modal de Ingreso -->
          <div :class="['modal', modalIngreso ? 'show d-block' : 'fade']" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="exampleModalLabel">Punto de Control</h5>
                  <button type="button" class="btn-close text-dark" @click="cerrarModal()"
                ...
                <div class="modal-body">
                  ...
                  <div class="form-group">
                    <label for="s_centro_id">Centro</label>
                    <input type="text" v-model="s_centro_id" class="form-control" id="s_centro_id">
                  </div>
                ...
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" @click="Save">Registrar Punto</button>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    listaActualizar() {
        this.$nextTick(async () => {
          try {
            await Promise.all([this.GET_DATA('sControles/filtro?filtro=' + this.user.trabajador_id)]).then((v) => {
              this.list = v[0];
            });
          } catch (e) {
            console.log(e);
    ...
    abrirIngreso() {
      ...
      this.iniciarMapa();
      this.iniciarCodigoQR(); // Iniciar el lector de códigos QR al abrir el modal
    },
    formatearHora() {
    ...
    async Save() {
      ...
      let self = this
      try {
        const formattedFechaNow = this.formatearHora(); // Usar la función de formateo
        const operacion = {
          fecha_hora: formattedFechaNow,
          s_centro_id: this.s_centro_id,
    ...
    Update() {
      let datos = {
        id: this.asistencias[0].id,
        s_centro_id: this.s_centro_id,
        latitud: this.latitud,
        longitud: this.longitud
      }
      this.$api.put(this.apiUrl + "/" + datos.id, datos).then(() => {
        this.listaActualizar();
        this.cerrarModal();
        this.$jc.alert("Salida registrada", "success");
      });
    },
    async iniciarMapa() {
      ...
      navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } })
        .then((stream) => {
          ...
          this.codeReader.decodeFromVideoDevice(null, 'video', (result, error) => {
            if (result) {
              this.s_centro_id = result.text;  // Asignar el valor detectado al v-model s_centro_id
              alert('Código QR detectado: ' + result.text);
              
              // Detener la lectura de QR y la cámara
              this.detenerCodigoQR();
            }
          });
        })
        .catch((err) => {
    ...
    detenerCodigoQR() {
      if (this.stream) {
        this.stream.getTracks().forEach(track => track.stop());  // Detener el stream de la cámara
      }
      if (this.codeReader) {
        this.codeReader.reset();  // Reiniciar el lector de códigos QR
      }
      const video = document.getElementById('video');
    ...
    cerrarModal() {
      ...
      this.detenerCodigoQR();  // Detener el lector al cerrar el modal
    },
