                ...
                <table class="table">
                  ...
                  <tbody>
                    <tr v-for="(m, i) in datosPaginados">
                      ...
                      <td data-label="Nombre" class="py-0 px-1">{{ m.empresa.nombre }}</td>
...
export default {
  ...
  data() {
    return {
      ...
      user_id:'',
      empresa_id:'',
      apiUrl: 'usersEmpresa/usuario',
      apiUrlSelect: 'usersEmpresa',
      page: 'Usuarios',
  ...
  mounted() {
    ...
    this.$nextTick(async () => {
      try {
        await Promise.all([this.GET_DATA(this.apiUrl + "?usuario=" + this.sl.sl_id), this.GET_DATA('empresas')]).then((v) => {
          this.list = v[0]
          this.empresas = v[1];
        })
      } catch (e) {
